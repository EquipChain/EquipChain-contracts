...
console.log('ðŸ” Validating Meter Simulator Structure...\n');
...
try {
  ...
  if (packageJson.name === 'Equipchain-meter-simulator') {
    console.log('âœ… Package name correct');
  } else {
    console.log('âŒ Package name incorrect');
    errors.push('Invalid package name');
  }
  
  if (packageJson.bin && packageJson.bin['meter-simulator']) {
    console.log('âœ… CLI binary defined');
  } else {
    console.log('âŒ CLI binary not defined');
  ...
  requiredDeps.forEach(dep => {
    if (packageJson.dependencies && packageJson.dependencies[dep]) {
      ...
      console.log(`âŒ Missing dependency: ${dep}`);
      errors.push(`Missing dependency: ${dep}`);
    }
  });
  
} catch (error) {
  console.log('âŒ Invalid package.json');
  errors.push('Invalid package.json');
}

// Validate main CLI file
console.log('\nðŸ”§ Validating main CLI file:');
try {
  const cliContent = fs.readFileSync('src/index.js', 'utf8');
  
  if (cliContent.includes('#!/usr/bin/env node')) {
    console.log('âœ… Shebang present');
  } else {
    console.log('âš ï¸  Shebang missing');
    warnings.push('Shebang missing');
  }
  
  if (cliContent.includes('program.version(\'1.0.0\')')) {
    console.log('âœ… Version defined');
  } else {
    console.log('âŒ Version not defined');
  ...
  requiredCommands.forEach(cmd => {
    if (cliContent.includes(`.command('${cmd}')`)) {
      ...
      console.log(`âŒ Missing command: ${cmd}`);
      errors.push(`Missing command: ${cmd}`);
    }
  });
  
} catch (error) {
  console.log('âŒ Invalid CLI file');
  errors.push('Invalid CLI file');
}

// Validate source files
console.log('\nðŸ“ Validating source files:');
const sourceFiles = [
  'src/config.js',
  'src/meter-device.js',
  'src/contract-interface.js',
...
sourceFiles.forEach(file => {
  try {
    ...
    if (content.includes('module.exports')) {
      console.log(`âœ… ${file} - Module exports present`);
    } else {
      console.log(`âŒ ${file} - No module exports`);
      errors.push(`No module exports in ${file}`);
    }
  } catch (error) {
    console.log(`âŒ ${file} - Invalid file`);
...
console.log(`âœ… Success: ${errors.length === 0 ? 'PASS' : 'FAIL'}`);
...
if (errors.length === 0) {
  console.log('\nðŸŽ‰ Meter Simulator structure validation PASSED!');
  process.exit(0);
} else {
  console.log('\nðŸ’¥ Meter Simulator structure validation FAILED!');
  process.exit(1);
}
