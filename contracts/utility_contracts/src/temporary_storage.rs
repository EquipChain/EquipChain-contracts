...
use crate::{ContinuousFlow, DataKey, Meter, ProviderWithdrawalWindow};
...
impl TempStorageManager {
    ...
    pub fn store_flow_accumulation(env: &Env, stream_id: u64, accumulation: i128, timestamp: u64) {
        env.storage()
            .temporary()
            .set(&TempStorageKey::FlowAccumulation(stream_id), &accumulation);
        env.storage()
            .temporary()
            .set(&TempStorageKey::FlowTimestamp(stream_id), &timestamp);
    }
    ...
    pub fn store_meter_usage_delta(env: &Env, meter_id: u64, usage_delta: i128, timestamp: u64) {
        env.storage()
            .temporary()
            .set(&TempStorageKey::MeterUsage(meter_id), &usage_delta);
        env.storage()
            .temporary()
            .set(&TempStorageKey::MeterLastUpdate(meter_id), &timestamp);
    }
    ...
    pub fn store_provider_window(env: &Env, provider: &Address, window: &ProviderWithdrawalWindow) {
        env.storage()
            .temporary()
            .set(&TempStorageKey::ProviderWindow(provider.clone()), window);
    }
    ...
    pub fn store_dust_delta(env: &Env, token: &Address, dust_delta: i128) {
        env.storage()
            .temporary()
            .set(&TempStorageKey::DustDelta(token.clone()), &dust_delta);
    }
    ...
    pub fn store_sla_delta(env: &Env, meter_id: u64, penalty_delta: u64) {
        env.storage()
            .temporary()
            .set(&TempStorageKey::SLADelta(meter_id), &penalty_delta);
    }
    ...
    pub fn store_fee_delta(env: &Env, stream_id: u64, fee_delta: i128) {
        env.storage()
            .temporary()
            .set(&TempStorageKey::FeeDelta(stream_id), &fee_delta);
    }
    ...
    pub fn store_batch_data<T: soroban_sdk::IntoVal<Env, soroban_sdk::Val>>(
        env: &Env,
        operation: Symbol,
        data: &T,
    ) {
        env.storage()
            .temporary()
            .set(&TempStorageKey::BatchOperation(operation), data);
    }

    /// Get batch operation data
    pub fn get_batch_data<T: soroban_sdk::TryFromVal<Env, soroban_sdk::Val>>(
        env: &Env,
        operation: Symbol,
    ) -> Option<T> {
        env.storage()
