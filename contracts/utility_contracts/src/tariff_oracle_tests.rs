use crate::tariff_oracle::{
    DailyTariffSchedule, HourlyTariff, TariffOracle, TariffOracleClient, TariffTier, HOURS_IN_DAY,
    TARIFF_NOTICE_PERIOD,
};
use crate::DataKey;
use soroban_sdk::{
    testutils::Address as _, testutils::BytesN as _, testutils::Ledger as _, Address, BytesN, Env,
    Vec,
};
...
pub mod tariff_oracle_tests {
    ...
    fn test_tariff_oracle_initialization() {
        ...
        env.register_contract(Some(&contract_address), TariffOracle);
    ...
    fn test_tariff_update_proposal() {
        ...
        let contract_address = Address::generate(&env);
        env.register_contract(Some(&contract_address), TariffOracle);
        let client = TariffOracleClient::new(&env, &contract_address);

        let grid_admin = Address::generate(&env);
        let initial_schedule = create_test_schedule(&env);

        env.mock_all_auths();
        client.initialize(&grid_admin, &initial_schedule);

        // Create new schedule proposal
        let new_schedule = create_peak_schedule(&env);
        let admin_signature = BytesN::random(&env);

        // Submit proposal
        let proposal_id = client.propose_tariff_update(&new_schedule, &admin_signature);

        // Verify proposal exists and is not executable yet
        let proposal = client.get_tariff_proposal(&proposal_id);
    ...
    fn test_tariff_update_execution() {
        ...
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            client.execute_tariff_update(&proposal_id);
        }));
        ...
        let current_schedule = client.get_current_schedule();
    ...
    fn test_current_flow_rate_calculation() {
        ...
        env.as_contract(&contract_address, || {
            ...
            let flow_rate =
                TariffOracle::calculate_current_flow_rate(env.clone(), consumption_rate);
    ...
    fn test_stream_spanning_midnight_blended_rate() {
        ...
        env.as_contract(&contract_address, || {
            ...
            let end_timestamp = 24 * 3600 + 60; // 00:01 next day
    ...
    fn test_renewable_energy_hours() {
        ...
        env.as_contract(&contract_address, || {
            ...
            // Check renewable hours (11-16 in our test schedule)
            for hour in 11..=16 {
    ...
    pub fn create_test_schedule(env: &Env) -> DailyTariffSchedule {
    ...
    pub fn create_peak_schedule(env: &Env) -> DailyTariffSchedule {
        let mut hourly_rates = Vec::new(env);
    ...
    pub fn create_invalid_schedule(env: &Env) -> DailyTariffSchedule {
        let mut hourly_rates = Vec::new(env);
    ...
    pub fn create_schedule_with_negative_rates(env: &Env) -> DailyTariffSchedule {
        let mut hourly_rates = Vec::new(env);
...
mod tariff_property_tests {
    use super::*;
    use crate::tariff_oracle_tests::tariff_oracle_tests::create_test_schedule;
    ...
    proptest! {
        ...
        fn test_flow_calculation_properties(
            ...
            env.as_contract(&contract_address, || {
                ...
                assert_eq!(result.duration_seconds, duration_minutes * 60);

                // Property: total tokens should be positive
                assert!(result.total_tokens > 0);

                // Property: weighted rate should be reasonable
                assert!(result.weighted_rate_per_second > 0);
                assert!(result.weighted_rate_per_second < 100000); // Sanity check

                // Property: windows_crossed should not exceed duration in hours
                let max_possible_windows = (duration_minutes / 60 + 1) as u8;
                assert!(result.windows_crossed <= max_possible_windows as u32);
            });
        }
    }
}
