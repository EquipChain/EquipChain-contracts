...
#[cfg(test)]
macro_rules! println { ($($tt:tt)*) => { std::println!($($tt)*) } }
#[cfg(test)]
macro_rules! format { ($($tt:tt)*) => { std::format!($($tt)*) } }
#[cfg(test)]
macro_rules! vec { ($($tt:tt)*) => { std::vec!($($tt)*) } }
...
use velocity_limit::{
    apply_override, get_velocity_config, revoke_override, set_velocity_config, VelocityDataKey,
};
#[contracttype]
#[derive(Clone, Debug, Eq, PartialEq)]
...
pub enum ContractError {
    ...
    FlowRateTooHigh = 109,
    YieldProtocolUnavailable = 110,
    YieldRoutingFailed = 111,
    InternalError = 112,
    UnauthorizedDevice = 113,
    NotFound = 114,
    NotInitialized = 115,
    FlowRateTooLow = 116,
...
fn validate_ed25519_public_key(env: &Env, public_key: &BytesN<32>) -> Result<(), ContractError> {
...
/// Ensures correct length and non-zero values
fn validate_ed25519_signature(env: &Env, signature: &BytesN<64>) -> Result<(), ContractError> {
...
fn validate_user_bytes(bytes: &Bytes, max_size: u32) -> Result<(), ContractError> {
...
fn is_green_source_verified(env: &Env, provider: &Address, meter_id: u64, timestamp: u64) -> bool {
    ...
    {
        let oracle_client = PriceOracleClient::new(env, &oracle_address);
        oracle_client.verify_green_source(&provider.clone(), &meter_id, &timestamp)
    } else {
        false
    }
}
...
fn issue_carbon_credits(
    ...
    env.events().publish(
        (symbol_short!("CarbCred"), meter_id),
...
// The `#[contract]` macro on `UtilityContract` already generates `UtilityContractClient`
// as a type alias at the crate root, so no re-export is needed here.
...
fn verify_groth16_proof(
    ...
    BytesN::<32>::from(proof_hash) != zero_hash
}
...
fn verify_zk_proof(env: &Env, proof_hash: BytesN<32>, challenge_data: &BytesN<32>) -> bool {
    ...
    BytesN::<32>::from(verification_result) != zero_hash
}
...
impl UtilityContract {
    ...
    pub fn add_supported_token(env: Env, token: Address) {
        ...
        env.events().publish((symbol_short!("TknSupp"),), token);
    }
    ...
    pub fn remove_supported_token(env: Env, token: Address) {
        ...
        env.events().publish((symbol_short!("TknUnsup"),), token);
    }
    ...
    pub fn add_supported_withdraw_token(env: Env, token: Address) {
        ...
        env.events().publish((symbol_short!("WdSupp"),), token);
    }
    ...
    pub fn remove_supported_withdraw_token(env: Env, token: Address) {
        ...
        env.events().publish((symbol_short!("WdUnsup"),), token);
    }
    ...
    pub fn revoke_velocity_override(env: Env, admin: Address, meter_id: u64) {
        ...
        let override_key = VelocityDataKey::VelocityOverride(meter_id);
        ...
        // Emit audit event
        env.events()
            .publish((symbol_short!("VelOverRv"),), (meter_id, admin));
    }
    ...
    pub fn add_sla_node(env: Env, admin: Address, node_pk: BytesN<32>) {
        ...
        if let Err(_) = validate_ed25519_public_key(&env, &node_pk) {
    ...
    /// Remove an SLA node from the registry
    pub fn remove_sla_node(
        ...
        validate_ed25519_public_key(&env, &node_pk)
    ...
    pub fn submit_sla_report(env: Env, signed_report: SignedSLAReport) {
        ...
        let report_xdr = report.clone().to_xdr(&env);
    ...
    pub fn initiate_device_transfer(
        ...
        transfer_data.append(&new_owner.clone().to_xdr(&env));
    ...
    pub fn complete_device_transfer(
        ...
        expected_transfer_data.append(&new_owner.clone().to_xdr(&env));
        let expected_transfer_id = env.crypto().sha256(&expected_transfer_data);

        if transfer_id != BytesN::<32>::from(expected_transfer_id) {
    ...
    pub fn complete_pairing(env: Env, meter_id: u64, signature: BytesN<64>) {
        ...
        validate_ed25519_signature(&env, &signature)
            .unwrap_or_else(|_| panic_with_error!(&env, ContractError::InvalidSignature));
    ...
    pub fn deduct_units(env: Env, signed_data: SignedUsageData) {
        ...
        validate_ed25519_signature(&env, &signed_data.signature)
            .unwrap_or_else(|_| panic_with_error!(&env, ContractError::InvalidSignature));
        validate_ed25519_public_key(&env, &signed_data.public_key)
            .unwrap_or_else(|_| panic_with_error!(&env, ContractError::InvalidSignature));
        ...
        let now = env.ledger().timestamp();
        let effective_rate = get_effective_rate(&env, &meter, signed_data.timestamp);
    ...
    pub fn propose_multisig_withdrawal(
        ...
        for i in 0..config.finance_wallets.len() {
            ...
            if env
                .try_invoke_contract::<(), crate::ContractError>(
                    &wallet,
    ...
    pub fn enable_privacy_mode(env: Env, meter_id: u64) {
        ...
        if !privacy_meters.contains(&meter_id) {
            ...
            // Rate limiting check
            let _ = check_stream_creation_rate_limit(&env, &meter.provider);
...
fn simulate_yield_routing_attempt(env: &Env, amount: i128) -> bool {
    ...
    let ledger_seq = env.ledger().sequence();
    let success_threshold = if amount > 1_000_000_000 {
        // Large amounts have lower success rate
        70 // 70% success for large amounts
    } else {
        90 // 90% success for normal amounts
    };

    (ledger_seq % 100) < success_threshold
}
    ...
    match error {
        ContractError::YieldProtocolUnavailable => {
            ...
            env.events().publish(
                ...
                (amount, Symbol::new(&env, "protocol_unavailable")),
            );
        ...
        ContractError::YieldRoutingFailed => {
            ...
            env.events().publish(
                (symbol_short!("Fallback2"),),
                (amount, Symbol::new(&env, "routing_failed")),
            );
        ...
        _ => {
            ...
            env.events().publish(
                (symbol_short!("NoRouting"),),
                (amount, Symbol::new(&env, "unexpected_error")),
            );
            0
        }
    }
...
fn attempt_alternative_routing(env: &Env, amount: i128) -> Result<i128, ContractError> {
    ...
    let ledger_seq = env.ledger().sequence();
    let fallback_success = (ledger_seq % 100) < 95; // 95% success rate

    if fallback_success {
        env.events().publish(
            (symbol_short!("AltRouted"),),
            (fallback_amount, Symbol::new(&env, "alternative_success")),
        );
        Ok(fallback_amount)
    } else {
...
fn attempt_partial_routing(env: &Env, amount: i128) -> Result<i128, ContractError> {
    ...
    let ledger_seq = env.ledger().sequence();
    let partial_success = (ledger_seq % 100) < 98; // 98% success rate

    if partial_success {
        env.events().publish(
            (symbol_short!("PartRoute"),),
            (partial_amount, Symbol::new(&env, "partial_success")),
        );
        Ok(partial_amount)
    } else {
