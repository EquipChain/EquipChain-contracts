...
mod tests {
    ...
    fn create_test_env() -> (Env, Address) {
        ...
        env.register_contract(Some(&contract_id), crate::UtilityContract);
        (env, contract_id)
    }
    ...
    fn test_flow_calculation_with_different_timestamps() {
        ...
        env.as_contract(&contract_id, || {
            ...
            let timestamp1 = 1500;
            let result1 =
                OptimizedFlowCalculator::calculate_with_temp_storage(&env, &flow, timestamp1);
            let expected1 = 100 * (timestamp1 - flow.last_flow_timestamp) as i128;
            assert_eq!(result1, expected1);

            let timestamp2 = 2000;
            let result2 =
                OptimizedFlowCalculator::calculate_with_temp_storage(&env, &flow, timestamp2);
    ...
    fn test_concurrent_temp_storage_operations() {
        ...
        env.as_contract(&contract_id, || {
            ...
            let result1 =
                OptimizedFlowCalculator::calculate_with_temp_storage(&env, &flow1, timestamp);
            let result2 =
                OptimizedFlowCalculator::calculate_with_temp_storage(&env, &flow2, timestamp);
            let result3 =
                OptimizedFlowCalculator::calculate_with_temp_storage(&env, &flow3, timestamp);
