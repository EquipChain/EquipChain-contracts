...
use crate::{
    validate_ed25519_public_key, validate_ed25519_signature, validate_sha256_hash, ContractError,
    DataKey,
};
...
impl NonceSyncManager {
    ...
    pub fn verify_heartbeat_nonce(env: Env, heartbeat: SignedHeartbeat) -> bool {
        ...
        validate_ed25519_signature(&env, &heartbeat.signature)
            .unwrap_or_else(|_| panic_with_error!(&env, ContractError::InvalidSignature));
        validate_ed25519_public_key(&env, &heartbeat.public_key)
            .unwrap_or_else(|_| panic_with_error!(&env, ContractError::PublicKeyMismatch));
        validate_sha256_hash(&heartbeat.device_mac)
            .unwrap_or_else(|_| panic_with_error!(&env, ContractError::InvalidSignature));
    ...
    pub fn reset_device_nonce(
        ...
        validate_sha256_hash(&device_mac)
            .unwrap_or_else(|_| panic_with_error!(&env, ContractError::InvalidSignature));
        ...
        // Add approval
        reset_request.approvals.push_back(approver.clone());

        // Check if we have enough approvals
        if reset_request.approvals.len() >= reset_request.required_approvals {
            // Execute reset
    ...
    pub fn initialize_device_nonce(env: Env, device_mac: BytesN<32>, initial_nonce: u64) {
        ...
        env.events()
            .publish((symbol_short!("NoncInit"),), (device_mac, initial_nonce));
    }
}
...
impl NonceSyncManager {
    ...
    fn handle_nonce_desync(
        ...
        env.events().publish((symbol_short!("NoncDsync"),), alert);
    ...
    fn is_authorized_resetter(env: &Env, address: &Address) -> bool {
        ...
        if let Some(resetters) = env
            .storage()
            .persistent()
            .get::<DataKey, Vec<Address>>(&resetters_key)
        {
