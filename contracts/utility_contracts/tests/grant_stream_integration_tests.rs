...
fn test_grant_stream_integration() {
    ...
    let grant_stream_contract = env.register(GrantStreamListener, ());
    let utility_contract = env.register(UtilityContract, ());

    // Initialize grant stream listener
    GrantStreamListener::init_listener(env.clone(), admin.clone(), treasury.clone());
